...
const systemTimezone = () => {
  ...
  try {
    ...
    // moment will parse the date differently if the tz is utc, so dont use
    // the system tz if its set to that
    if (tz.includes("UTC")) {
      tz = null
    }
  } catch (e) {
...
const stringifyEvents = (events) =>
  JSON.stringify(events.map((event) => _.omit(event, "__raw")))
...
class ScoreEmitter extends EventEmitter {
  constructor(options) {
    ...
    if (!interval || typeof interval !== "number" || interval < ms("1m")) {
      throw new Error(
        "Interval must be a number of milliseconds no less than 1 second"
      )
    }
  ...
  getCurrentDate() {
    return this.getCurrentMoment()
      .subtract(this.options.dailyCutoff, "m")
      .format("YYYYMMDD")
  }
  ...
  fetch(options) {
    ...
    this.logger.info(
      "[DATE]",
      JSON.stringify({
        now: moment(this.options.__now || void 0),
        moment: this.getCurrentMoment(),
        date: date,
        cutoff: this.options.dailyCutoff,
  ...
  parse(allEvents, options) {
    ...
    const available = filteredEvents.filter(
      (event) => this.__skippedIds.indexOf(event.id) === -1
    )

    const found = _.filter(
      available,
      completedOnly ? this.options.completed : _.stubTrue
    )
    const eventsLeft = available.filter((event) =>
      completedOnly
        ? !_.find(found, { id: event.id })
    ...
    if (!completedOnly && shouldEmit) {
      this.logger.info("[EVENTS]", JSON.stringify(found))
    ...
    const allUnstarted = eventsLeft.every(
      (event) => event.status.state === "pre"
    )
    const anyInProgress = eventsLeft.find(
      (event) => event.status.state === "in"
    )
