...
class ScoreEmitter extends EventEmitter {
  ...
  parse(allEvents, options) {
    ...
    const eventsLeft = notParsed.length - newlyCompleted.length;
    ...
    if (notParsed.length === 0 || eventsLeft === 0) {
    ...
    else if (notParsed.every((event) => event.status.state === 'pre')) {
      this.logger.info('[NEXT]', 'Waiting for first event');
      this.nextFetch(_.minBy(notParsed, 'status.timeUntil').status.timeUntil);
    }
    // Wait for the time in the next closest game to the end
    else {
      this.logger.info('[NEXT]', 'Waiting for next event to finish');
      this.nextFetch(_.minBy(notParsed, 'status.timeRemaining').status.timeRemaining);
    }
  }
