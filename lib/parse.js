...
const overwrite = (obj, omitKeys, newObj) =>
  Object.assign({}, _.omit(obj, omitKeys), newObj)
const omitKeys = {
...
const parseRaw = (e) =>
  overwrite(e, omitKeys.event, {
    competitions: e.competitions.map((c) =>
      overwrite(c, omitKeys.competition, {
        competitors: c.competitors.map((co) =>
          overwrite(co, omitKeys.competitor, {
...
const parseCompetitor = (c) => {
  ...
  const name = _.unescape(
    _.get(c, "team.displayName", "")
      .replace(_.get(c, "team.name", ""), "")
      .trim()
  )
...
const parseSeries = (s) => {
  const playedCompetitions = _.sumBy(
    _.get(s, "summary", "1-0").match(/\d+/g),
    (n) => +n
  )
  ...
  const completed = _.get(
    s,
    "completed",
    playedCompetitions === totalCompetitions
  )
...
const parseTimeRemaining = (s) => {
  ...
  const periodLength =
    typeof s.periodLength === "string" ? ms(s.periodLength) : s.periodLength
  ...
  const futurePeriodClock =
    !finalPeriod || !periodLength || period >= finalPeriod
      ? 0
...
const parseStatus = (s) => {
  ...
  // 2021-03-20 Oregon  - VCU was uncontested but not picked up because the status was
  // not shows as "completed". For our purposes the game was completed so explicitly set it
  // as completed
  else if (
    state === STATES.POST &&
    !completed &&
    s.type.name === "STATUS_UNCONTESTED"
  ) {
    completed = true
  }
  ...
  return {
    ...
    timeUntil:
      state === "pre"
        ? parseTime(moment(s.date).diff(moment(__now || void 0))) +
          parseTime(parseTimeRemaining(s))
        : null,
  }
}
...
const parseEvent = (e, options) => {
  ...
  const status = parseStatus(
    Object.assign({ date, hasWinner }, options, e.status)
  )
  ...
  return {
    ...
    seriesCompleted:
      typeof series.completed === "boolean"
        ? series.completed && status.completed
        : status.completed,
    __raw: parseRaw(e),
  }
}
...
const parseScores = (url, options, cb) => {
  ...
  return jsdom.env(url, {
    ...
    features: {
      ...
      return closeCb(
        null,
        events.map((e) => parseEvent(e, options))
      )
    },
  })
}
