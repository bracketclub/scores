...
const urls = dates.map((date) => `http://espn.go.com/${event}-college-basketball/scoreboard/_/date/${date}`);

async.map(urls, (url, cb) => {
  parse(url, (err, events) => {
    if (err) return cb(err);

    return cb(null, _.transform(events, (res, e) => {
      res.push(
  ...
  const all = _.chain(results)
    ...
    .mapValues((teams) => {
      if (mapValues) {
        return teams.map((t) => `${t.rank} - ${t.name} - (${_.without(t.names, t.name).join(', ')})`);
      }
      if (namesOnly) {
