  ...
  function processScores($, $els) {
    ...
    $els.each(function(idx, item) {
      ...
      matchups.each(function(jdx, gameDiv) {
        ...
        if(status.match('final')) {
          status = 'final';
          game.status = status;
        } else if(status.match(/[\d]:[\d]{2}/)) {
        ...
        game.home.score = parseInt($(gameDiv).find('.home ul.score li.final').text());
        game.visitor.score = parseInt($(gameDiv).find('.visitor ul.score li.final').text());

        gameId = game.home.team.replace(/\s/ig, '') + 'v' + game.visitor.team.replace(/\s/ig, '') + moment().format('YYYYMMDD');

        if(games.hasOwnProperty(gameId) &&
           _.compare(games[gameId], game)) {
