...
var ScoreTracker = function(options) {
  ...
  function processRegion(gameNote) {
    var region = (gameNote.match(/ ([A-Za-z]*) REGION /) || [])[1] || null,
        finalFour = (gameNote.match(/FINAL FOUR/) || [])[0] || null,
        championship = (gameNote.match(/NATIONAL CHAMPIONSHIP/) || [])[0] || null;

    return region || finalFour || championship || null;
  }
    ...
    $els.each(function(idx, item) {
      ...
      matchups.each(function(jdx, gameDiv) {
        ...
        var game = {
          ...
          status: 'scheduled',
          winner: false,
          region: processRegion($(gameDiv).find('.game-note').text()),
          home: {
            team: $(gameDiv).find('.home .team-name span a').attr('title'),
            seed: parseInt($(gameDiv).find('.home .team-name span:first-child').text(), 10) || null,
            score: null
          },
          visitor: {
            team: $(gameDiv).find('.visitor .team-name span a').attr('title'),
            seed: parseInt($(gameDiv).find('.visitor .team-name span:first-child').text(), 10) || null,
            score: null
          }
        };
