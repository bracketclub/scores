...
test("scheduled games for the next day", (t) => {
  ...
  utils.parseFile("20160310-zero-started", options, (err, events) => {
    ...
    t.equal(
      events[0].status.timeUntil,
      ms("15h") + GAME_LENGTH * utils.LENGTH_COEFFICIENT
    )
    t.equal(
      events[1].status.timeUntil,
      ms("15h") + GAME_LENGTH * utils.LENGTH_COEFFICIENT
    )
    t.equal(
      events[2].status.timeUntil,
      ms("15h") + GAME_LENGTH * utils.LENGTH_COEFFICIENT
    )
    t.equal(
      events[3].status.timeUntil,
      ms("15.5h") + GAME_LENGTH * utils.LENGTH_COEFFICIENT
    )
...
test("scheduled games for the next day with a different time", (t) => {
  ...
  utils.parseFile("20160310-zero-started", options, (err, events) => {
    ...
    t.equal(
      events[3].status.timeUntil,
      ms("15m") + GAME_LENGTH * utils.LENGTH_COEFFICIENT
    )
